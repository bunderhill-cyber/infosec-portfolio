<#
.SYNOPSIS
    Detects suspicious execution and defense-evasion activity related to LOLBins and AMSI bypass attempts (v2).

.DESCRIPTION
    Hunts for common indicators of Living-Off-the-Land binary abuse and AMSI bypass techniques.
    Uses Sysmon (preferred) and Windows PowerShell Script Block logging.
    Designed as the defensive counterpart to Invoke-LolbinDemo.ps1 and Invoke-AmsiBypassDemo.ps1.

    Version 2 improvements:
    - Stronger detection of reflection-based AMSI bypass techniques
    - Additional AMSI-related patterns in both process creation and script block logs
    - Updated after initial lab testing showed the original version missed the AMSI demo
...
.NOTES
    ...
    Requires: Administrator rights + Sysmon strongly recommended.
    Lab / authorized use only.

.EXAMPLE
    .\Detect-SuspiciousExecution_v2.ps1
    .\Detect-SuspiciousExecution_v2.ps1 -TimeWindowMinutes 15 -VerboseOutput
#>
...
param(
    ...
    [string]$OutputPath = ".\SuspiciousExecution_v2_$(Get-Date -Format 'yyyyMMdd_HHmm').csv",
...
function Write-ScriptLog {
    ...
    $LogMessage | Out-File -FilePath ".\Detect-SuspiciousExecution_v2.log" -Append -Encoding UTF8
}

Write-ScriptLog "Starting suspicious execution hunt (v2). Looking back $TimeWindowMinutes minutes."
...
try {
    ...
    if (Get-WinEvent -ListLog $sysmonLog -ErrorAction SilentlyContinue) {
        ...
        foreach ($event in $events) {
            try {
                ...
                if ($image -match '\\msiexec\.exe$' -and $commandLine -match '/q') {
                    $reason += "msiexec quiet install pattern"
                }
                ...
                # --- Improved AMSI bypass indicators (command line) ---
                if ($commandLine -match 'AmsiUtils' -or
                    $commandLine -match 'AmsiScanBuffer' -or
                    $commandLine -match 'amsiInitFailed' -or
                    $commandLine -match 'amsi\.dll' -or
                    $commandLine -match '\[Ref\]\.Assembly\.GetTypes' -or
                    $commandLine -match 'SetValue\(\$null,\s*\[IntPtr\]::Zero\)' -or
                    $commandLine -match 'System\.Management\.Automation\.AmsiUtils') {
    ...
    # ============================================
    # 2. PowerShell Script Block Logging (4104) - Improved
    # ============================================
    ...
    if (Get-WinEvent -ListLog $psLog -ErrorAction SilentlyContinue) {
        ...
        foreach ($event in $psEvents) {
            try {
                ...
                # Stronger AMSI bypass detection in script blocks
                if ($msg -match 'AmsiUtils' -or
                    $msg -match 'AmsiScanBuffer' -or
                    $msg -match 'amsiInitFailed' -or
                    $msg -match '\[Ref\]\.Assembly\.GetTypes' -or
                    $msg -match 'GetFields\([''"]NonPublic,Static[''"]\)' -or
                    $msg -match 'SetValue\(\$null,\s*\[IntPtr\]::Zero\)' -or
                    $msg -match 'System\.Management\.Automation\.AmsiUtils') {
                ...
                # LOLBin references inside script blocks
                if ($msg -match 'mshta' -or $msg -match 'rundll32' -or $msg -match 'regsvr32' -or $msg -match 'wmic') {
                ...
                if ($reason.Count -gt 0) {
                    $findings += [PSCustomObject]@{
                        ...
                        CommandLine = ($msg -replace "`r|`n", " ").Substring(0, [Math]::Min(350, $msg.Length))
                        ParentImage = "N/A"
                        User        = "N/A"
                        ProcessId   = "N/A"
    ...
    Write-ScriptLog "Suspicious execution hunt (v2) completed."
}